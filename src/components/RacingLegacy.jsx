...
const pilotProfiles = [
  ...
  },
  {
    ...
    stats: ["Leyenda del Automovilismo", "Múltiple Campeón Nacional", "Constructor de Éxitos"]
  },
  {
    id: "froilan",
    ...
    stats: ["1° Victoria Ferrari F1", "Subcampeón Mundial F1", "Ganador de Le Mans"]
  },
  {
    id: "fontana",
  ...
  },
  {
    ...
    stats: ["Campeón de TC 1968", "Hito con el Trueno Naranja", "Protagonista de Película"]
  },
  {
    id: "trosset",
...
const wallItems = [
  { id: 1, title: "Agustín Canapino", media: "/Canap.mp4", type: "video", poster: "/estaca.jpg", isVertical: true },
  { id: 2, title: "", media: "/auto.jpg", type: "img", isVertical: false },
  { id: 3, title: "", media: "/sector-automovilismo.jpg", type: "img", isVertical: false },
  { id: 4, title: "", media: "/canapino-parri.jpg", type: "img", isVertical: false },
  { id: 5, title: "", media: "/canapino-auto.jpg", type: "img", isVertical: false },
  { id: 6, title: "", media: "/quique.jpg", type: "img", isVertical: false },
  { id: 7, title: "", media: "/parri-copas.jpg", type: "img", isVertical: false },   
];
...
export default function RacingLegacy() {
  ...
  return (
    <section id="automovilismo" className="relative overflow-hidden bg-wood-dark py-24 text-stone-100">
      ...
      <div className="relative mx-auto max-w-7xl px-6">
        ...
        <div className="mb-12">
          <div className="mb-4 flex items-center gap-3 text-brand-cream">
            ...
            <span className="text-sm font-bold uppercase tracking-[0.3em] italic">Cuna de Campeones</span>
          </div>
        ...
        <div className="mb-20 space-y-8">
          ...
          <div className="flex items-center justify-between lg:hidden px-4 mb-2">
            <span className="text-[10px] uppercase tracking-[0.2em] text-stone-500 font-bold">Desliza para ver pilotos</span>
            <motion.div animate={{ x: [0, 5, 0] }} transition={{ repeat: Infinity, duration: 1.5 }}>
              <MoveRight size={16} className="text-brand-red" />
            </motion.div>
          </div>

          {/* Selector de Chips */}
          <div className="relative">
            {/* Gradientes laterales de desvanecimiento (Mobile Only) */}
            <div className="absolute left-0 top-0 bottom-0 w-12 bg-gradient-to-r from-wood-dark to-transparent z-20 pointer-events-none lg:hidden" />
            <div className="absolute right-0 top-0 bottom-0 w-12 bg-gradient-to-l from-wood-dark to-transparent z-20 pointer-events-none lg:hidden" />

            <div className="flex overflow-x-auto py-6 pb-10 px-10 lg:px-2 lg:flex-wrap lg:justify-start gap-5 scrollbar-hide snap-x snap-mandatory overflow-y-visible">
              {pilotProfiles.map((pilot) => (
                ...
                >
                  <div className="h-12 w-12 overflow-hidden rounded-xl border border-white/20 pointer-events-none">
                    <img src={pilot.image} alt={pilot.name} className="h-full w-full object-cover" />
                  </div>
                  <div className="text-left pointer-events-none">
                    <p className={`text-[14px] font-bold leading-none ${activePilot.id === pilot.id ? 'text-brand-cream' : 'text-stone-300'}`}>
                      {pilot.name}
                    </p>
            ...
            >
              <div className="lg:col-span-4 flex justify-center lg:justify-start">
                <div className="relative aspect-square w-full max-w-[340px] overflow-hidden rounded-[2.5rem] border border-white/10 shadow-2xl">
                  <img src={activePilot.image} alt={activePilot.name} className="h-full w-full object-cover" />
              ...
              <div className="lg:col-span-8 space-y-6 text-center lg:text-left">
                <div>
                  <div className="flex items-center justify-center lg:justify-start gap-3 text-brand-red mb-4">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.4em]">Legado de Arrecifes</span>
                  </div>
                  <h3 className="font-display text-5xl text-brand-cream italic lg:text-7xl">{activePilot.name}</h3>
                  <p className="text-stone-400 text-xl font-medium italic mt-2">{activePilot.title}</p>
                </div>
                ...
                <div className="flex flex-wrap justify-center lg:justify-start gap-4 pt-4">
                  {activePilot.stats.map((stat, idx) => (
                    <div key={idx} className="flex items-center gap-3 bg-white/5 border border-white/10 px-6 py-4 rounded-3xl">
                      <Trophy size={18} className="text-brand-red" />
                      <span className="text-xs font-bold uppercase tracking-widest text-stone-200">{stat}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="pt-16 border-t border-white/5">
          <h4 className="text-[10px] font-black uppercase tracking-[0.5em] text-stone-500 mb-10 text-center lg:text-left">Archivo Visual de Boxes</h4>
              ...
              >
                <div className="h-full w-full relative">
                  {item.type === "video" ? (
                    ...
                    <img 
                      src={item.media} 
                      alt={item.title} 
                      className={`${item.isVertical ? "h-full w-full object-contain bg-black/40" : "h-full w-full object-cover"} grayscale-[0.5] group-hover:grayscale-0 transition-all duration-700`} 
                    />
                  )}
                ...
                {item.title && (
                  <div className="absolute bottom-0 left-0 p-8">
                    <p className="font-display text-2xl text-brand-cream translate-y-2 group-hover:translate-y-0 transition-all duration-300">{item.title}</p>
                  </div>
                )}
              </motion.div>
            ))}
          ...
          >
            <motion.div
              ...
              className={`relative bg-stone-900 rounded-[3rem] overflow-hidden shadow-2xl border border-white/10 ${
                selectedMedia.isVertical ? "max-w-md w-full" : "max-w-6xl w-full"
              }`}
              onClick={(e) => e.stopPropagation()}
            >
              <button onClick={() => setSelectedMedia(null)} className="absolute right-6 top-6 z-50 p-3 bg-black/60 text-brand-cream rounded-full hover:bg-brand-red transition-all">
                <X size={24} />
              </button>
              {selectedMedia.type === "video" ? (
                <video src={selectedMedia.media} className="w-full h-auto max-h-[85vh] object-contain" controls autoPlay />
              ) : (
                <img src={selectedMedia.media} alt={selectedMedia.title} className="w-full h-auto max-h-[85vh] object-contain" />
              )}
            </motion.div>
          </motion.div>
        )}
